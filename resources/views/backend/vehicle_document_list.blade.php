            ...
            <div class="row mb-4">
                ...
                <div class="col-sm-6">
                    <div class="float-right">
                        <a href="{{ route('add_vehicle_document') }}">
                            <button type="button" class="btn btn-primary" id="add_btn" style="float:right ; "><i
                                    class="fa fa-plus-circle"></i> &nbsp;Add Vehicle Document &nbsp;</button>
                        </a>
                    </div>
                </div><!-- /.col -->
            </div><!-- /.row -->
    ...
    <script>
        ...
        function show_project() {
            ...
            $("#vehicle_repairing").dataTable({
                ...
                serverSide: true,
                "ajax": {
                    url: 'show_vehicle_document_list',
                    type: 'post',
                    data: {
                        '_token': '{{ csrf_token() }}',
                    },
            ...
            $("#vehicle_repairing").on('draw.dt', function() {
                $(".edit_btn").click(function() {
                    ...
                    redirectPost("{{ url('edit_vehicle_document') }}", datas);
                ...
                function delete_data(code) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data.status == 3) {
                                var msg =
                                    "<strong>Success:</strong> Vehicle Document details deleted Successfully";
                                $.confirm({
                                    title: 'Success !',
                                    type: 'green',
                                    icon: 'fa fa-check',
                ...
                $('.view_pdf').click(function() {
                    var upload_file = this.id;
                    // alert(upload_file);
                    // alert("hhhh");
                    var str = "<iframe src='uploads/document/" + upload_file +
                        "' width='100%' height='970px'></iframe>";
                    $.confirm({
                        title: 'PDF',
                        type: 'blue',
                        icon: "fa fa-eye",
